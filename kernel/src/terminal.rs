...
// Position of terminal right now
static mut X: u64 = 0;
static mut Y: u64 = 0;

// some macro that help format print and other things
#[macro_export]
macro_rules! print {
    ($($arg:tt)*) => {{
...
// for formatting and writing to graphics
pub struct Terminal;

// override and change the implementation for writing to graphics
...
// function to write char to graphics
pub fn printc(c: char) {

    // if new \n then add Y and return for exit the function
    if c == '\n' {
        unsafe {
            Y += FONT_H as u64;
            X = 0;
    ...
    // take the char as usize to use it as index to bitmap to which the character we take
    let bitmap_index: usize = c as usize;
    let bitmap = FONT8X16_BASIC[bitmap_index];

    // Loop through height and width of FONT
    if let Some(resp) = FRAMEBUFFER_REQUEST.get_response() {
        if let Some(fb) = resp.framebuffers().next() {
            ...
            for y in 0..FONT_H {
                for x in 0..FONT_W {
                    let bit: u8 = bitmap[y as usize] & (1 << (7-x));
                    if bit != 0 {
                        unsafe {
...
pub fn scrolling() {
    if let Some(resp) = FRAMEBUFFER_REQUEST.get_response() {
        if let Some(fb) = resp.framebuffers().next() {
            ...
            let offset = FONT_H as u64 * pitch;
            let bytes = (height - FONT_H as u64) * pitch;
            unsafe {
                let src = fb.addr().add(offset as usize);
                let dst = fb.addr();
            
                ptr::copy(src, dst, bytes as usize);
            }
...
// the FONT is bitmap
const FONT8X16_BASIC:[[u8; 16]; 128] = [
